...
const RotationsContainer = () => {
  ...
  return (
    <div className="container mx-auto p-6 space-y-8">
      <Card>
        <CardHeader>
          <CardTitle className="text-2xl font-bold text-primary">Rotation Centers</CardTitle>
          <CardDescription>Create the list of centers for your rotations</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="flex space-x-2 mb-4">
            ...
            />
            <Button onClick={handleAddCenter} className="bg-primary hover:bg-primary-700">
              <Plus className="mr-2 h-4 w-4" /> Add Center
            </Button>
          </div>
          {centers.length > 0 && (
            <Button className="bg-secondary hover:bg-secondary-700 mb-5" onClick={() => {setCenters([])}}>Clear All</Button>
          )}
          <div className="columns-4">
            {centers.map((center, index) => (
              <div key={center.id} className="flex items-center bg-gray-100 rounded p-6">
                ...
                <Button variant="ghost" size="sm" onClick={() => handleRemoveCenter(center.id)}>
