  ...
  const handleDeleteCenterList = async () => {
    try {
      ...
      if (result.success) {
        setSavedCenters(
          savedCenters.filter(
            (centerList) => centerList.id !== currentCenterListId
          )
        );
    ...
    <div className="container mx-auto p-6 space-y-8">
      {!isFinalized && (
        <div className="mb-4">
          <h2 className="text-2xl font-bold text-primary mb-2">
            My Saved Center Rotations
          </h2>
          <div className="flex space-x-2">
            {savedCenters.length === 0 ? (
              <p className="text-gray-500 italic">
                You don't have any saved centers yet! Save some for quick and easy
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {isEditing ? "Edit Title for Centers" : "Set Title for Centers"}
            </DialogTitle>
          </DialogHeader>
